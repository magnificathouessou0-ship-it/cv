...
// Récupérer TOUTES les relations d'amitié (envoyées et reçues)
$relations_amis = $bdd->prepare("
    SELECT 
        CASE 
    ...
    AND statut IN ('en_attente', 'accepter')
");
$relations_amis->execute([$user_id, $user_id, $user_id]);
$amis_relations = $relations_amis->fetchAll(PDO::FETCH_KEY_PAIR);
    ...
    <div class="card mb-4">
        <div class="card-body">
            <ul class="nav nav-pills justify-content-center mb-0 gap-2">
                <li class="nav-item">
                    <a class="nav-link <?= $tab === 'explorer' ? 'active' : '' ?>" href="?tab=explorer" style="color: #000000 !important;">
                        <i class="fas fa-search me-1"></i>Explorer
                    </a>
                </li>
                <li class="nav-item">
                    <a class="nav-link <?= $tab === 'invitations' ? 'active' : '' ?>" href="?tab=invitations" style="color: #000000 !important;">
                ...
                <li class="nav-item">
                    <a class="nav-link <?= $tab === 'blocages' ? 'active' : '' ?>" href="?tab=blocages" style="color: #000000 !important;">
                                ...
                                <div class="card h-100 user-card">
                                    <div class="card-body text-center">
                                        ...
                                        <div class="d-flex flex-column gap-2">
                                            <?php if (isset($amis_relations[$u['id']])): ?>
                                                <?php if ($amis_relations[$u['id']] == 'en_attente'): ?>
                                                    <button class="btn btn-warning btn-sm w-100" disabled>
                                                        <i class="fas fa-clock me-1"></i>En attente
                                                    </button>
                                                <?php elseif ($amis_relations[$u['id']] == 'accepter'): ?>
                                            ...
                                            <!-- Bouton Bloquer : ouvre le modal -->
                                            <button type="button"
                                                    class="btn btn-outline-danger btn-sm w-100 block-btn"
                                                    data-user-id="<?= $u['id'] ?>"
                                                    data-user-name="<?= htmlspecialchars($u['prenom'] . ' ' . $u['nom']) ?>">
                    ...
                    <div class="list-group">
                        <?php foreach ($bloques as $b): ?>
                            <div class="list-group-item d-flex justify-content-between align-items-center">
                                ...
                                <!-- Bouton Débloquer : ouvre modal -->
                                <button type="button"
                                        class="btn btn-warning btn-sm unblock-btn"
                                        data-user-id="<?= $b['id_bloque'] ?>"
                                        data-user-name="<?= htmlspecialchars($b['prenom'] . ' ' . $b['nom']) ?>">
...
<!-- MODALS POUR CONFIRMATIONS BLOQUER / DÉBLOQUER -->
<!-- Modal Bloquer -->
<div class="modal fade" id="blockModal" tabindex="-1" aria-labelledby="blockModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-header bg-danger text-white">
        <h5 class="modal-title" id="blockModalLabel"><i class="fas fa-ban me-2"></i>Bloquer l'utilisateur</h5>
        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Fermer"></button>
      </div>
      <div class="modal-body">
        <p>Voulez-vous vraiment bloquer <strong id="blockUserName"></strong> ?</p>
        <p class="small text-muted">L'utilisateur ne pourra plus voir votre profil ni vous envoyer des messages.</p>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Annuler</button>
        <a href="#" id="confirmBlockBtn" class="btn btn-danger">Bloquer</a>
      </div>
    </div>
  </div>
</div>
  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-header bg-warning text-dark">
        <h5 class="modal-title" id="unblockModalLabel"><i class="fas fa-unlock me-2"></i>Débloquer l'utilisateur</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Fermer"></button>
      </div>
      <div class="modal-body">
        <p>Voulez-vous vraiment débloquer <strong id="unblockUserName"></strong> ?</p>
        <p class="small text-muted">L'utilisateur retrouvera la possibilité de voir votre profil et de vous contacter.</p>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Annuler</button>
        <a href="#" id="confirmUnblockBtn" class="btn btn-warning">Débloquer</a>
      </div>
    </div>
  </div>
</div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.querySelectorAll('.block-btn').forEach(btn => {
        btn.addEventListener('click', function () {
            ...
            // définir l'URL de confirmation (GET comme ton backend l'attend)
            document.getElementById('confirmBlockBtn').setAttribute('href', 'explorer.php?block=' + encodeURIComponent(userId) + '&tab=explorer');
    ...
    document.querySelectorAll('.unblock-btn').forEach(btn => {
        btn.addEventListener('click', function () {
            ...
            // définir l'URL de confirmation (GET comme ton backend l'attend)
            document.getElementById('confirmUnblockBtn').setAttribute('href', 'explorer.php?unblock=' + encodeURIComponent(userId) + '&tab=blocages');
